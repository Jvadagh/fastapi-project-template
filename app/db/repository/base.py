...
from sqlalchemy import nulls_last, asc, BinaryExpression, desc, delete, update, func, text
...
async def create_list_of_entities(session: AsyncSession, entities: list):
    try:
...
async def update_entity(
        ...
        entity_type,  # TODO: Specify the type
        to_update: dict,
        commit: bool = True,
...
async def total_count_of_entities_by_filter(
        session: AsyncSession,
        entity_type,  # TODO: Specify the type
        filters: List[BinaryExpression] = None,
) -> int:
    if filters is None:
        filters = list()
    query = select(func.count(entity_type.id))
    for query_filter in filters:
        query = query.where(query_filter)
...
async def create_bulk_entity(
        session: AsyncSession,
        entity_type,  # TODO: Specify the type
        entities: List[dict]
):
    query = insert(entity_type).values(entities).on_conflict_do_nothing()
